...
export interface ConfessionRepository {
  ...
  save(confession: Partial<ConfessionEntity> & { collegeId: string; title: string; content: string }): Promise<ConfessionEntity>;
  updateStatus(id: string, collegeId: string, status: ConfessionEntity['status']): Promise<void>;
  listFeed(collegeId: string, options: { categoryCode?: string; tab?: 'trending' | 'latest'; cursor?: string; limit?: number }): Promise<ConfessionEntity[]>;
}

export interface CommentRepository {
  findById(id: string, collegeId: string): Promise<CommentEntity | null>;
  save(comment: Partial<CommentEntity> & { collegeId: string; confessionId: string; content: string }): Promise<CommentEntity>;
...
export interface ModerationRepository {
  ...
  saveCase(modCase: Partial<ModerationCaseEntity> & { collegeId: string; confessionId: string; severityLevel: number }): Promise<ModerationCaseEntity>;
  recordAction(action: { collegeId: string; caseId: string; moderatorUserId: string; action: string; reasonNote?: string }): Promise<void>;
...
export interface NotificationRepository {
  queueNotification(notification: { collegeId: string; recipientUserId: string; notificationType: string; payloadJson: string }): Promise<void>;
}
...
export interface VoteRepository {
  addConfessionVote(confessionId: string, voterUserId: string, voteType: 'UPVOTE' | 'DOWNVOTE', collegeId: string): Promise<void>;
  removeConfessionVote(confessionId: string, voterUserId: string, collegeId: string): Promise<void>;
  getUserConfessionVote(confessionId: string, voterUserId: string, collegeId: string): Promise<'UPVOTE' | 'DOWNVOTE' | null>;
}

export interface MediaRepository {
  attachMedia(confessionId: string, mediaUrl: string, mediaType: string, mimeType: string, collegeId: string): Promise<void>;
}
