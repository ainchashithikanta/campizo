...
export function registerContributorRoutes(app: FastifyInstance, deps: ContributorControllerDependencies): void {
  ...
  app.get('/api/v1/contributors/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
  ...
  app.get('/api/v1/contributors/:id/resources', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    return reply.send({
      success: true,
      data: [],
      meta: { requestId: request.headers['x-request-id'] || 'unknown', timestamp: new Date().toISOString() }
  ...
  app.get('/api/v1/contributors/:id/collections', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    return reply.send({
      success: true,
      data: [],
      meta: { requestId: request.headers['x-request-id'] || 'unknown', timestamp: new Date().toISOString() }
