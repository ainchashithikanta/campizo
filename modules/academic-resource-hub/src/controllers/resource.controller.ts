...
export function registerResourceRoutes(app: FastifyInstance, deps: ResourceControllerDependencies): void {
  ...
  app.get('/api/v1/resources/:resourceId', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
  ...
  // 6. Publish Resource
  app.patch('/api/v1/resources/:resourceId/publish', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
  ...
  // 7. Archive Resource
  app.patch('/api/v1/resources/:resourceId/archive', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
  ...
  // 8. Replace Resource
  app.patch('/api/v1/resources/:resourceId/replace', async (request: FastifyRequest<{ Params: { resourceId: string }; Body: { newVersionId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
      const resource = await deps.replaceResourceUC.execute(request.params.resourceId, collegeId, request.body.newVersionId);
      return reply.send({
        success: true,
        data: resource,
        meta: { requestId: request.headers['x-request-id'] || 'unknown', timestamp: new Date().toISOString() }
  ...
  app.delete('/api/v1/resources/:resourceId', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
  ...
  app.get('/api/v1/resources/:resourceId/versions', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    return reply.send({
      success: true,
      data: [],
      meta: { requestId: request.headers['x-request-id'] || 'unknown', timestamp: new Date().toISOString() }
  ...
  // 11. Create Version
  app.post('/api/v1/resources/:resourceId/versions', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
  ...
  // 12. Publish Version
  app.patch('/api/v1/resources/:resourceId/versions/:versionId/publish', async (request: FastifyRequest<{ Params: { resourceId: string; versionId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
      const resource = await deps.publishVersionUC.execute(request.params.resourceId, request.params.versionId, collegeId);
      return reply.send({
        success: true,
        data: resource,
        meta: { requestId: request.headers['x-request-id'] || 'unknown', timestamp: new Date().toISOString() }
  ...
  // 13. Rollback Version
  app.patch('/api/v1/resources/:resourceId/versions/:versionId/rollback', async (request: FastifyRequest<{ Params: { resourceId: string; versionId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    try {
      const resource = await deps.rollbackVersionUC.execute(request.params.resourceId, request.params.versionId, collegeId);
      return reply.send({
        success: true,
        data: resource,
        meta: { requestId: request.headers['x-request-id'] || 'unknown', timestamp: new Date().toISOString() }
  ...
  app.post('/api/v1/resources/:resourceId/bookmarks', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    const userId = (request.headers['x-user-id'] as string) || 'guest-user-101';
    try {
  ...
  app.delete('/api/v1/resources/:resourceId/bookmarks', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    return reply.send({
      success: true,
  ...
  app.post('/api/v1/resources/:resourceId/votes', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
  ...
  // 17. Remove Vote
  app.delete('/api/v1/resources/:resourceId/votes', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    return reply.send({
      success: true,
  ...
  // 18. Report Resource
  app.post('/api/v1/resources/:resourceId/reports', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
  ...
  // 19. Record Download
  app.post('/api/v1/resources/:resourceId/download', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    const userId = (request.headers['x-user-id'] as string) || 'guest-user-101';
    try {
  ...
  // 20. Record View
  app.post('/api/v1/resources/:resourceId/view', async (request: FastifyRequest<{ Params: { resourceId: string } }>, reply: FastifyReply) => {
    const collegeId = (request.headers['x-college-id'] as string) || 'default-college';
    const userId = (request.headers['x-user-id'] as string) || 'guest-user-101';
    try {
