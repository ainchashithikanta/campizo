...
export default function QuestionBankPage() {
  ...
  const filtered = questions.filter((q) => {
    const matchesSearch = !searchQuery || q.questionText.toLowerCase().includes(searchQuery.toLowerCase()) || q.companyName.toLowerCase().includes(searchQuery.toLowerCase()) || q.topic.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <main className="max-w-6xl mx-auto p-6 font-sans">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Real interview questions asked across tech companies with difficulty, topic tags, and frequency counts.</p>
        </div>
        <Link
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
        <div className="flex-1 w-full">
          <SearchBar value={searchQuery} onChange={setSearchQuery} placeholder="Search question text, company, or topic (e.g. Word Ladder, Graphs)..." />
        </div>
      ...
      {!loading && !error && filtered.length > 0 && (
        <div className="space-y-4">
          {filtered.map((q) => (
            <article key={q.id} className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-2">
                <div>
                  <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">{q.companyName} • {q.roleTitle}</span>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">#{q.topic}</span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${q.difficulty === 'HARD' ? 'bg-rose-500/10 text-rose-600' : 'bg-amber-500/10 text-amber-600'}`}>
                      {q.difficulty}
                    </span>
