...
import { fetchNotificationDigests, generateNotificationDigest, type NotificationDigestJob } from '../../../lib/api-notifications';
    ...
    <main className="max-w-4xl mx-auto p-6 font-sans">
      ...
      {!loading && digests.length === 0 && (
        <div className="p-12 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50/50">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 mb-4">You have no archived notification digests. Click above to bundle unread updates.</p>
        </div>
      )}
      ...
      {!loading && digests.length > 0 && (
        <div className="space-y-4">
          {digests.map((d) => (
            <div key={d.id} className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Covering period from {new Date(d.periodStart).toLocaleDateString()} to {new Date(d.periodEnd).toLocaleDateString()}
                </p>
              </div>
