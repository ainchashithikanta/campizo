...
export default function FeatureDetailPage({ params }: { params: Promise<{ featureKey: string }> }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'evaluation' | 'dependencies' | 'rollouts' | 'approvals' | 'snapshots' | 'analytics' | 'audit' | 'danger'>('overview');
      ...
      <div className="ff-card-panel">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            ...
            <p style={{ color: '#94A3B8', fontSize: '13px', marginTop: '4px' }}>Marketplace P2P Buyer-Seller Direct Messaging</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', borderBottom: '1px solid #334155', paddingBottom: '12px' }}>
          {(['overview', 'evaluation', 'dependencies', 'rollouts', 'approvals', 'snapshots', 'analytics', 'audit', 'danger'] as const).map((t) => (
            <button
              key={t}
        ...
        {activeTab === 'overview' && (
          <div>
            ...
            <p style={{ color: '#94A3B8', fontSize: '13px', marginTop: '8px' }}>Owner Team: Team Marketplace | Version: v4 | Lifecycle: PRODUCTION</p>
          </div>
        )}
        ...
        {activeTab === 'danger' && (
          <div style={{ border: '1px solid #EF4444', padding: '16px', borderRadius: '8px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#94A3B8', marginBlock: '8px' }}>Archiving or removing a feature flag requires confirmation.</p>
            <button className="ff-btn-danger" onClick={() => confirm(`Are you sure you want to archive ${featureKey}?`) && alert('Feature archived.')}>
