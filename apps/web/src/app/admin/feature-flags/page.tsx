...
export default function FeatureFlagsOpsConsole() {
  const [activeTab, setActiveTab] = useState<'overview' | 'explorer' | 'rollout' | 'topology' | 'snapshots' | 'approvals' | 'telemetry' | 'audit' | 'packs' | 'bulk'>('overview');
  ...
  const [activityStream, setActivityStream] = useState<Array<{ id: string; time: string; text: string; type: 'info' | 'warning' | 'danger' }>>([
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') {
        ...
        const tabs: Array<typeof activeTab> = ['overview', 'explorer', 'rollout', 'topology', 'snapshots', 'approvals', 'telemetry', 'audit', 'packs'];
  ...
  return (
    <div className="ff-ops-console">
        ...
        <div className="ff-header-top">
          <div className="ff-brand">
            ...
            <h1 className="ff-title">
              ⚡ Feature Management Operations Console
            </h1>
          </div>
      ...
      <nav className="ff-nav-tabs">
        <button className={`ff-tab-btn ${activeTab === 'overview' ? 'active' : ''}`} onClick={() => setActiveTab('overview')}>
          📊 Overview
        </button>
        <button className={`ff-tab-btn ${activeTab === 'explorer' ? 'active' : ''}`} onClick={() => setActiveTab('explorer')}>
          🚩 Feature Flags <span className="ff-tab-pill">{flags.length}</span>
        </button>
        <button className={`ff-tab-btn ${activeTab === 'rollout' ? 'active' : ''}`} onClick={() => setActiveTab('rollout')}>
          🚀 Canary Rollouts
        </button>
        <button className={`ff-tab-btn ${activeTab === 'topology' ? 'active' : ''}`} onClick={() => setActiveTab('topology')}>
          🕸️ Dependency Graph
        </button>
        <button className={`ff-tab-btn ${activeTab === 'snapshots' ? 'active' : ''}`} onClick={() => setActiveTab('snapshots')}>
          📸 Snapshots & Restore
        </button>
        <button className={`ff-tab-btn ${activeTab === 'approvals' ? 'active' : ''}`} onClick={() => setActiveTab('approvals')}>
          🛡️ 4-Eye Approvals <span className="ff-tab-pill">1</span>
        </button>
        <button className={`ff-tab-btn ${activeTab === 'telemetry' ? 'active' : ''}`} onClick={() => setActiveTab('telemetry')}>
          📈 Telemetry & Health
        </button>
      ...
      <main className="ff-content">
          ...
          <div>
            <div className="ff-grid-4">
              ...
              <div className="ff-card">
                ...
                <div className="ff-stat-num" style={{ color: '#10B981' }}>0.14 ms</div>
              ...
              <div className="ff-card">
                ...
                <div className="ff-stat-num" style={{ color: '#F59E0B' }}>1</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
              ...
              <div className="ff-card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {flags.map((flag) => (
                    <div key={flag.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: 'var(--ff-surface-elevated)', borderRadius: '8px' }}>
                      <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <span className="ff-stat-num" style={{ fontSize: '14px', color: '#06B6D4' }}>0.11 ms</span>
              ...
              <div className="ff-card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '420px', overflowY: 'auto' }}>
                  {activityStream.map((log) => (
                    <div key={log.id} style={{ padding: '10px 12px', borderRadius: '6px', background: 'var(--ff-surface-elevated)', borderLeft: `4px solid ${log.type === 'danger' ? '#EF4444' : log.type === 'warning' ? '#F59E0B' : '#6366F1'}` }}>
                      <div style={{ fontSize: '11px', color: 'var(--ff-text-muted)', fontFamily: 'var(--ch-font-mono)' }}>{log.time}</div>
                      <div style={{ fontSize: '12px', marginTop: '2px', color: 'var(--ff-text-bright)' }}>{log.text}</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <tr key={flag.id} onClick={() => setSelectedFlag(flag)}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ flex: 1, height: '6px', background: 'var(--ff-surface-elevated)', borderRadius: '3px', width: '80px', overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${flag.rolloutPercentage}%`, background: 'var(--ff-primary)' }}></div>
                        </div>
                        <span style={{ fontSize: '11px', fontFamily: 'var(--ch-font-mono)' }}>{flag.rolloutPercentage}%</span>
                      </div>
                    </td>
        ...
        {activeTab === 'rollout' && (
          <div className="ff-card">
            ...
            <p style={{ fontSize: '13px', color: 'var(--ff-text-muted)', marginBottom: '24px' }}>
              Safely increment rollout percentage buckets with target cohort controls and instant kill-switch protection.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              <div style={{ background: 'var(--ff-surface-elevated)', padding: '20px', borderRadius: '10px' }}>
                ...
                <div style={{ padding: '16px', background: 'var(--ff-surface-dark)', borderRadius: '8px', marginBottom: '16px' }}>
                  <div style={{ fontSize: '12px', color: 'var(--ff-text-muted)' }}>Estimated Cohort Impact:</div>
                  <div style={{ fontSize: '24px', fontWeight: 700, color: '#06B6D4', fontFamily: 'var(--ch-font-mono)' }}>
                    ~{(14500 * (simTargetPercentage / 100)).toLocaleString()} / 14,500 active students
                  </div>
                </div>

                <button
                  style={{ width: '100%', background: 'linear-gradient(135deg, #10B981, #059669)', color: '#FFF', fontWeight: 700, padding: '12px', borderRadius: '8px' }}
              ...
              <div style={{ background: 'var(--ff-surface-elevated)', padding: '20px', borderRadius: '10px' }}>
                ...
                <ul style={{ fontSize: '13px', display: 'flex', flexDirection: 'column', gap: '8px', color: 'var(--ff-text-muted)' }}>
                  <li>✔ Murmur3 Deterministic Hash Bucket matching</li>
                  <li>✔ Environment override guard: <code>PRODUCTION</code></li>
                  <li>✔ App Version requirement: <code>&gt;= 2.4.0</code></li>
                  <li>✔ Role restriction: <code>VERIFIED_STUDENT</code></li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="ff-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div>
                ...
                <p style={{ fontSize: '12px', color: 'var(--ff-text-muted)' }}>Kahn's Topological Sort cycle detection active. 0 cycles detected.</p>
              </div>
              <button style={{ background: 'var(--ff-surface-elevated)', padding: '8px 14px', borderRadius: '8px', border: '1px solid var(--ff-surface-border)', fontSize: '12px' }}>
            ...
            <div className="ff-graph-canvas">
              <div style={{ display: 'flex', alignItems: 'center', gap: '40px' }}>
                <div className="ff-node-card">
                  ...
                  <div className="ff-flag-key" style={{ fontSize: '14px', marginTop: '4px' }}>marketplace.user_auth</div>
                  <div className="ff-badge ff-badge-prod" style={{ marginTop: '6px' }}>Rank 0</div>
                </div>
                ...
                <div className="ff-node-card" style={{ borderColor: 'var(--ff-primary)' }}>
                  ...
                  <div className="ff-flag-key" style={{ fontSize: '14px', marginTop: '4px' }}>marketplace.p2p_chat</div>
                  <div className="ff-badge ff-badge-prod" style={{ marginTop: '6px' }}>Rank 1</div>
                </div>
                ...
                <div className="ff-node-card">
                  ...
                  <div className="ff-flag-key" style={{ fontSize: '14px', marginTop: '4px' }}>ai.study_assistant</div>
                  <div className="ff-badge ff-badge-beta" style={{ marginTop: '6px' }}>Rank 2</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="ff-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                ...
                <p style={{ fontSize: '12px', color: 'var(--ff-text-muted)' }}>Immutable point-in-time state backups with 1-click restore.</p>
              </div>
              <button
                style={{ background: 'var(--ff-primary)', color: '#FFF', padding: '10px 16px', borderRadius: '8px', fontWeight: 600, fontSize: '13px' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                { id: 'snap_1722714488', env: 'PRODUCTION', flags: 6, time: '2026-08-03 19:50:00', hmac: 'hmac_sha256_88a91c', note: 'Pre-deployment baseline' },
                { id: 'snap_1722680000', env: 'PRODUCTION', flags: 6, time: '2026-08-03 12:00:00', hmac: 'hmac_sha256_44b20f', note: 'Daily automated backup' }
              ].map((snap) => (
                <div key={snap.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: 'var(--ff-surface-elevated)', borderRadius: '10px' }}>
                  <div>
                    <div style={{ fontFamily: 'var(--ch-font-mono)', fontWeight: 700, color: 'var(--ff-primary)' }}>{snap.id}</div>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button style={{ background: 'var(--ff-surface-dark)', border: '1px solid var(--ff-surface-border)', color: 'var(--ff-text-bright)', padding: '6px 12px', borderRadius: '6px', fontSize: '12px' }}>
                      Compare Diff
                    </button>
                    <button style={{ background: 'var(--ff-warning)', color: '#000', fontWeight: 700, padding: '6px 12px', borderRadius: '6px', fontSize: '12px' }} onClick={() => alert(`Restored snapshot ${snap.id}`)}>
                      Restore Snapshot
                    </button>
                  </div>
                </div>
        ...
        {activeTab === 'approvals' && (
          <div className="ff-card">
            ...
            <div style={{ background: 'var(--ff-surface-elevated)', padding: '20px', borderRadius: '10px', borderLeft: '4px solid var(--ff-warning)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--ff-warning)', fontWeight: 700 }}>PENDING REVIEW</div>
                  <h4 style={{ fontSize: '15px', marginTop: '4px' }}>Enable <code>marketplace.p2p_chat</code> in PRODUCTION</h4>
                  <div style={{ fontSize: '12px', color: 'var(--ff-text-muted)', marginTop: '4px' }}>
                    Requested by <strong>dev_lead_01</strong> | Ticket: <code>REQ-8841</code> | Policy: 4-Eye Double Approval
                  </div>
                </div>

                <div style={{ display: 'flex', gap: '10px' }}>
                  <button style={{ background: 'var(--ff-danger)', color: '#FFF', fontWeight: 700, padding: '8px 14px', borderRadius: '6px', fontSize: '12px' }} onClick={() => alert('Change ticket REJECTED.')}>
                    Reject Ticket
                  </button>
                  <button style={{ background: 'var(--ff-success)', color: '#FFF', fontWeight: 700, padding: '8px 14px', borderRadius: '6px', fontSize: '12px' }} onClick={() => alert('Change ticket APPROVED.')}>
                    Approve & Deploy
                  </button>
                </div>
              </div>
        ...
        {activeTab === 'telemetry' && (
          <div className="ff-grid-4">
            ...
            <div className="ff-card">
              ...
              <div className="ff-stat-num" style={{ color: '#06B6D4' }}>1.4 ms</div>
          ...
          <div className="ff-card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[
                ...
                <div key={idx} style={{ padding: '12px', background: 'var(--ff-surface-elevated)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <span style={{ fontSize: '11px', fontFamily: 'var(--ch-font-mono)', color: 'var(--ff-text-muted)' }}>{item.time}</span>
                    <div style={{ fontWeight: 600, fontSize: '13px', marginTop: '2px' }}>
                      {item.actor} performed <code>{item.action}</code> on <code>{item.flag}</code>
                    </div>
                  </div>
                  <span style={{ fontSize: '11px', fontFamily: 'var(--ch-font-mono)', color: 'var(--ff-accent-cyan)' }}>{item.hash}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="ff-card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ padding: '16px', background: 'var(--ff-surface-elevated)', borderRadius: '10px' }}>
                ...
                <p style={{ fontSize: '12px', color: 'var(--ff-text-muted)', marginTop: '4px' }}>Groups: <code>marketplace.p2p_chat</code>, <code>marketplace.payment</code></p>
                <button style={{ marginTop: '12px', background: 'var(--ff-primary)', color: '#FFF', padding: '6px 12px', borderRadius: '6px', fontSize: '12px' }} onClick={() => alert('Pack deployed.')}>
                  Deploy Pack
                </button>
              </div>
              ...
              <div style={{ padding: '16px', background: 'var(--ff-surface-elevated)', borderRadius: '10px' }}>
                ...
                <p style={{ fontSize: '12px', color: 'var(--ff-text-muted)', marginTop: '4px' }}>Enforces 10% canary rollout with 4-Eye Approval requirement</p>
                <button style={{ marginTop: '12px', background: 'var(--ff-surface-dark)', color: '#FFF', border: '1px solid var(--ff-surface-border)', padding: '6px 12px', borderRadius: '6px', fontSize: '12px' }}>
                  Apply Template
                </button>
              </div>
            </div>
        ...
        {activeTab === 'bulk' && (
          <div className="ff-card">
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button style={{ background: 'var(--ff-success)', color: '#FFF', padding: '10px 16px', borderRadius: '8px', fontWeight: 600 }} onClick={() => alert('Bulk ENABLE completed for selected flags.')}>
                Bulk Enable Selected (3)
              </button>
              <button style={{ background: 'var(--ff-warning)', color: '#000', padding: '10px 16px', borderRadius: '8px', fontWeight: 600 }} onClick={() => alert('Bulk DISABLE completed for selected flags.')}>
                Bulk Disable Selected (3)
              </button>
              <button style={{ background: 'var(--ff-danger)', color: '#FFF', padding: '10px 16px', borderRadius: '8px', fontWeight: 600 }} onClick={() => alert('Bulk DEPRECATE completed.')}>
                Bulk Deprecate (3)
              </button>
            </div>
          </div>
      ...
      {selectedFlag && (
        <>
          ...
          <div className="ff-drawer">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 className="ff-flag-key" style={{ fontSize: '16px' }}>{selectedFlag.flagKey}</h3>
              <button onClick={() => setSelectedFlag(null)} style={{ fontSize: '20px', color: 'var(--ff-text-muted)' }}>✕</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontSize: '13px' }}>
              ...
              <div style={{ padding: '16px', background: 'var(--ff-surface-elevated)', borderRadius: '8px' }}>
                ...
                <ol style={{ fontSize: '11px', color: 'var(--ff-text-muted)', paddingLeft: '16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
        ...
        <div className="ff-modal-backdrop" onClick={() => setIsCommandPaletteOpen(false)}>
          <div className="ff-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
              <div style={{ padding: '10px', background: 'var(--ff-surface-elevated)', borderRadius: '6px', cursor: 'pointer' }} onClick={() => { setActiveTab('explorer'); setIsCommandPaletteOpen(false); }}>
                🚩 Go to Feature Flags Explorer
              </div>
              <div style={{ padding: '10px', background: 'var(--ff-surface-elevated)', borderRadius: '6px', cursor: 'pointer' }} onClick={() => { setIsKillSwitchModalOpen(true); setIsCommandPaletteOpen(false); }}>
                🚨 Trip Emergency Kill Switch (Shift + K)
              </div>
              <div style={{ padding: '10px', background: 'var(--ff-surface-elevated)', borderRadius: '6px', cursor: 'pointer' }} onClick={() => { setActiveTab('snapshots'); setIsCommandPaletteOpen(false); }}>
      ...
      {isKillSwitchModalOpen && (
        <div className="ff-modal-backdrop" onClick={() => setIsKillSwitchModalOpen(false)}>
          <div className="ff-modal" onClick={(e) => e.stopPropagation()} style={{ borderColor: 'var(--ff-danger)' }}>
            <h3 style={{ fontSize: '18px', color: 'var(--ff-danger)', marginBottom: '8px' }}>🚨 Emergency Kill Switch Control</h3>
            <p style={{ fontSize: '12px', color: 'var(--ff-text-muted)', marginBottom: '16px' }}>
              Tripping an emergency kill switch immediately forces a feature treatment to OFF across all production nodes in &lt;100ms.
            </p>
            ...
            <select id="ks-select" className="ff-env-select" style={{ width: '100%', marginBottom: '16px' }}>
              {flags.map((f) => (
                <option key={f.id} value={f.flagKey}>{f.flagKey} ({f.module})</option>
              ))}
            </select>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button
                style={{ background: 'var(--ff-surface-elevated)', color: '#FFF', padding: '10px 16px', borderRadius: '8px', border: '1px solid var(--ff-surface-border)' }}
                onClick={() => setIsKillSwitchModalOpen(false)}
              >
                Cancel
              </button>
              <button
                style={{ background: 'var(--ff-danger)', color: '#FFF', fontWeight: 700, padding: '10px 20px', borderRadius: '8px' }}
                onClick={() => {
                  const sel = (document.getElementById('ks-select') as HTMLSelectElement)?.value || flags[0]?.flagKey || '';
        ...
        <div className="ff-modal-backdrop" onClick={() => setIsCreateModalOpen(false)}>
          <div className="ff-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateFlag} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                <label style={{ fontSize: '12px', color: 'var(--ff-text-muted)', display: 'block', marginBottom: '4px' }}>
              ...
              <div>
                ...
                <select className="ff-env-select" style={{ width: '100%' }} value={newModule} onChange={(e) => setNewModule(e.target.value)}>
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '10px' }}>
                <button type="button" style={{ background: 'var(--ff-surface-elevated)', color: '#FFF', padding: '10px 16px', borderRadius: '8px' }} onClick={() => setIsCreateModalOpen(false)}>
                  Cancel
                </button>
                <button type="submit" style={{ background: 'var(--ff-primary)', color: '#FFF', fontWeight: 700, padding: '10px 20px', borderRadius: '8px' }}>
