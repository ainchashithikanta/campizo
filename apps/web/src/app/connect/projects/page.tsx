...
export default function ProjectTeamsPage() {
  ...
  const loadProjects = () => {
    fetchProjectTeams()
      .then((res) => {
        setProjects(res || [
          { id: 'proj_101', title: 'TreeHacks AI Assistant Team', description: 'Seeking PyTorch & Next.js developers for 36-hour hackathon', status: 'OPEN' }
        ]);
        setLoading(false);
      })
      .catch((err) => {
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <form onSubmit={handleCreate} className="bg-white dark:bg-slate-900 p-6 rounded-2xl max-w-md w-full border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
            ...
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full min-h-[44px] px-3 border rounded-xl" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Description & Requirements</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full p-3 border rounded-xl text-xs" />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" className="min-h-[44px] px-4 py-2 text-xs font-semibold rounded-xl bg-emerald-600 text-white">Publish Team</button>
            </div>
          </form>
        </div>
      )}
