    ...
    <ol className="relative border-l border-slate-200 dark:border-slate-800 ml-3 space-y-6">
      {activities.map((act) => (
        <li key={act.activityId} className="ml-6">
          ...
          <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{act.actionType.replace('_', ' ')}</h4>
...
export function PrivacyPanel({ settings, onToggle }: { settings: PrivacySettings; onToggle: (key: 'isGhostMode' | 'isIncognitoMode', value: boolean) => void }) {
  return (
    <div className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Hide your student profile from all discovery feeds and recommendations.</p>
        </div>
        <button
          type="button"
          role="switch"
          ...
          className={`min-h-[44px] min-w-[50px] px-3 py-1 rounded-full text-xs font-bold transition-colors ${
            settings.isGhostMode ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
          }`}
        >
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Browse intent listings anonymously without recording profile view history.</p>
        </div>
        <button
          type="button"
          role="switch"
          ...
          className={`min-h-[44px] min-w-[50px] px-3 py-1 rounded-full text-xs font-bold transition-colors ${
            settings.isIncognitoMode ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
          }`}
        >
