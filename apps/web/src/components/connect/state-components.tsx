...
export function EmptyState({ title = 'No Items Found', description = 'There are no items available right now.' }: { title?: string; description?: string }) {
  return (
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-800/50 animate-pulse h-44" />
      ))}
    </div>
  );
}
