...
6. **Publish**: Progress bar fills $\rightarrow$ Confirmation screen: *"Material live! Earned +10 Contributor Points."*
...
6. **Action Bar (Sticky Bottom on Mobile)**:
   ...
   - `🚩 Report`
7. **Version History & File Lineage**: Shows if a newer version of the file or syllabus scheme exists (e.g. *"Updated version uploaded on Aug 2024"*).
...
- **Instant Hash Matching**: If the file's SHA-256 hash matches an existing document in the college repository, the upload halts immediately with a warning: *"This exact file has already been uploaded by Priya N. View existing document."*
...
2. **Quarantine Notice**: Flagged files display a non-intrusive warning: *"This document has been reported by 3 students for wrong categorization and is under review."*
